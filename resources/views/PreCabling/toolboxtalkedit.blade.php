...
<form action="{{route('PreCabling.toolboxtalk.update',$toolboxtalk['id'])}}" method="POST" enctype="multipart/form-data">
                @csrf
        @if(isset($siteSurvey))
            @method('PUT')
@endif
...
<div class="row">
    ...
    <div class="form-group">
        ...
        <label for="picture_during_toolbox_yes">
            <input type="radio" id="picture_during_toolbox_yes" name="picture_during_toolbox"  onclick="toggleImageInputs(true)" value="yes" style="appearance: radio; -webkit-appearance: radio; -moz-appearance: radio; width: auto; display: inline-block; margin-right: 5px;" {{ ($toolboxtalk->picture_during_toolbox ?? old('picture_during_toolbox', 'no')) === 'yes' ? 'checked' : '' }}>
            Yes
        </label>
        <label for="picture_during_toolbox_no">
            <input type="radio" id="picture_during_toolbox_no" name="picture_during_toolbox"  onclick="toggleImageInputs(false)" value="no" style="appearance: radio; -webkit-appearance: radio; -moz-appearance: radio; width: auto; display: inline-block; margin-right: 5px;" {{ ($toolboxtalk->picture_during_toolbox ?? old('picture_during_toolbox', 'no')) === 'no' ? 'checked' : '' }}>
            No
        </label>
    </div>
</div>
...
<div id="hidden-button-container" style="display: none;"></div>
...
<form action="{{ route('PreCabling.toolboxtalk.destroy', $toolboxtalk->id) }}" method="POST" style="display: inline;">
    @csrf
    @method('DELETE')
    <button type="submit" class="btn btn-danger">Delete Toolbox</button>
</form>
